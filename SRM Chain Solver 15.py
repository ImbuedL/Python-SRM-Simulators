   ...
   """
    ...
    while True:
        ...
        for room_load_list in all_room_load_lists:
            ...
            Allocate_Hookshot(Heap, 0, Overlay_Dict) # 0 is the room_number of Room0
            Load_Room(Heap, Room_List[1], Room_List[0].priority_queue[1], Overlay_Dict)
            Allocate_Bomb(Heap, 1, Overlay_Dict) # 1 is the room_number of Room1
            Allocate_Bomb(Heap, 1, Overlay_Dict) # 1 is the room_number of Room1
            Allocate_Charged_Spin_Attack(Heap, 1, Overlay_Dict) # 1 is the room_number of Room1
            Load_Room(Heap, Room_List[0], Room_List[0].priority_queue[1], Overlay_Dict)
            Allocate_Charged_Spin_Attack(Heap, 0, Overlay_Dict)
            Load_Room(Heap, Room_List[1], Room_List[0].priority_queue[1], Overlay_Dict)
            Allocate_Smoke(Heap, 1, Overlay_Dict) # superslide with smoke still allocated
            Load_Room(Heap, Room_List[0], Room_List[0].priority_queue[1], Overlay_Dict)
            Load_Room(Heap, Room_List[1], Room_List[0].priority_queue[1], Overlay_Dict)
            ...
            for transition in room_load_list[0:len(room_load_list)-1]:
                ...
                elif room_count == 0 and len(Heap) > 2:
                   
                    """
                   
                   In this case we are assuming that we are starting after a specific
                   sequence of actions has taken place and thus the Room that we are
                   in is already loaded, so we don't want to load the Room. All we want
                   to do is perform deallocation/allocation steps
...
# CHANGE PATH TO WHERE YOU HAVE YOUR TEXT FILE SAVED
save_path = "C:\\Users\\xyzabc\\Documents\\Bizhawk RAM Watch\\scripts\\"
name_of_file = "Grave Chain 0"
...
Randomized_Solver(Room1, Room_List, 2, Allocation_List_Dict_Grave_0, Grabbable_dict, Overlay_dict, complete_name, [0x160, 0x1F0], Heap)
 
"""
 
THIS COPY OF THE SCRIPT HARDCODES THE INITIAL HEAP INTO THE SOLVER FUNCTION TO LOOK
FOR SRM CHAINS IN BENEATH THE GRAVEYARD
